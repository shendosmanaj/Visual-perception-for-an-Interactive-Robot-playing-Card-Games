...
card_model.compile(loss="sparse_categorical_crossentropy", optimizer=opt, metrics=["accuracy"])
...
opt = tf.keras.optimizers.Adam(lr=0.0005, beta_1=0.9, beta_2=0.999,epsilon=1e-7)
FER_model.compile(loss="sparse_categorical_crossentropy", optimizer=opt, metrics=["accuracy"])
...
def YOLO():
    ...
    cap = cv2.VideoCapture("MOV_2400.mp4") # Uncomment this if you want to run it on a video. Change the path to the video first.
    # cap.set(3, 1280)
    # cap.set(4, 720)


    vid_cod = cv2.VideoWriter_fourcc(*'XVID')
    out = cv2.VideoWriter("cam_video.avi", vid_cod, 10.0,
        ...
        if len(rects) > 0:
            for (fX, fY, fW, fH) in rects:
                ...
                for (i, (emotion, prob)) in enumerate(zip(Emotion_classes, preds)):
                    ...
                    cv2.putText(frame_resized, text, (fX, fY - (20 * i) - 25), cv2.FONT_HERSHEY_SIMPLEX, 0.45,
                                (255, 255, 255), 2)
