"""
module for generating a dataset with decks of cards.
This dataset is used for training the model for card detection.
The cards are saved as images together with their labels and the bounding boxes as text files.
This dataset will enable the yolo algorithm(implemented with Darknet Framework) to find the positions of the cards
on the table, and label them as Covered or Uncovered.
"""
...
def shuffled_cards(images, m=4, n=4):
    """
    Make two pairs of each card, shuffle and reshape them.
    """
...
def cards_to_remove_indices(images):
    ...
    removed_cards_indices = [
        (i, j)
        for i, row in enumerate(images)
        for j, elem in enumerate(row)
        if elem == removed_cards
    ]
    ...
    for i in range(0, len(cards)):
        for j in range(0, len(cards[i])):
            if (i, j) not in removed_cards_indices:
                ...
                x, y = area
                x_center, y_center, width, height = data_yolo_format(
                    x, y, card_width, card_height, table_width, table_height
                )
                labels += (
                        card_type
                        + " "
